...
public class MicroClimateDataMsgHandler extends AbstractMsgHandler {
    ...
    public void send2IOT(IOTMqttClient mqttClient){
        ...
        //发送位置数据
        Document lcDataDocument = new Document();
        lcDataDocument.put("longitude", deviceMsg.getLongitude());
        lcDataDocument.put("latitude", deviceMsg.getLatitude());
        lcDataDocument.put("altitude", 0);
        Document lcDocument = new Document();
        lcDocument.put("deviceid", deviceMsg.getDeviceId());
        lcDocument.put("location", lcDataDocument);
        lcDocument.put("ts", String.valueOf(deviceMsg.getMsgDate().getTime()));
        String lcMsg = lcDocument.toJson();
        mqttClient.sendMsg (topic, lcMsg);
        //发送采集数据
        ...
        parentDocument.put("ts", String.valueOf(deviceMsg.getMsgDate().getTime()));
...
//        int latitudeFlag = 1;
        ...
        int latitudeSec = ParseUtil.combineData(msgQue.pop(), msgQue.pop(), msgQue.pop(), msgQue.pop());
//        if(latitudeSec < 0){
...
//        int longitudeFlag = 1;
        ...
        int longitudeSec = ParseUtil.combineData(msgQue.pop(), msgQue.pop(), msgQue.pop(), msgQue.pop());
//        if(longitudeSec < 0){
...
//取测量数据
        int rain5Min = ParseUtil.combineData(msgQue.pop(), msgQue.pop(), msgQue.pop(), msgQue.pop());
        deviceMsg.setRain5Min(rain5Min/10.0);
        int rain24Hour = ParseUtil.combineData(msgQue.pop(), msgQue.pop(), msgQue.pop(), msgQue.pop());
        deviceMsg.setRain24Hour(rain24Hour/10.0);
        int rainLastDay = ParseUtil.combineData(msgQue.pop(), msgQue.pop(), msgQue.pop(), msgQue.pop());
        ...
        deviceMsg.setTemperature((temperature-500)/10.0);
        //照度
        int illumination = ParseUtil.combineData(msgQue.pop(), msgQue.pop(), msgQue.pop(), msgQue.pop());
        deviceMsg.setIllumination(illumination);
        //气压
        int airPressure = ParseUtil.combineData(msgQue.pop(), msgQue.pop(), msgQue.pop(), msgQue.pop());
        deviceMsg.setAirPressure(airPressure);
        //风速
        int windSpeed = ParseUtil.combineData(msgQue.pop(), msgQue.pop());
        deviceMsg.setWindSpeed(windSpeed/10.0);
        //风向
        int windDirection = ParseUtil.combineData(msgQue.pop(), msgQue.pop());
        deviceMsg.setWindDirection(windDirection/10.0);
        //电池电压
        int cellVoltage = ParseUtil.combineData(msgQue.pop(), msgQue.pop());
